...
"""
def test(img_A, path='300_2520_G.pth'):
    start = time.time()
    ...
    result[:, :, 0]=de_norm(fake_B.detach()[0]).numpy()[0]
    result[:, :, 1]=de_norm(fake_B.detach()[0]).numpy()[1]
    result[:, :, 2]=de_norm(fake_B.detach()[0]).numpy()[2]
